...
void GetNextReport(USB_JoystickReport_Input_t* const ReportData) {
	...
	if (buf_button & (0x10)) // pin 4 = L R
        {
            ReportData->Button |= AllButton[4];
			ReportData->Button |= AllButton[5];
        }

	if (buf_button & (0x08)) // pin 0 = Rstick Left
        {
            ReportData->RX = 0; // Right Stick X-Axis (Neutral)
    		ReportData->RY = 128; // Right Stick Y-Axis (Neutral)
        }
	else
		{
			ReportData->RX = 128; // Right Stick X-Axis (Neutral)
    		ReportData->RY = 128; // Right Stick Y-Axis (Neutral)
		}



	if (buf_button & (0x02)) // pin 2 = A
        {
            ReportData->Button |= AllButton[2];
        }

	if (buf_button & (0x01)) // pin 3 = Up
        {
            ReportData->LY = 255;
			ReportData->LX = 128;
        }
    	...
    	{
        	// If D3 is not pressed, initialize LY to neutral (128) before checking other axis inputs
       		ReportData->LY = 128;
			ReportData->LX = 128;
    	}
